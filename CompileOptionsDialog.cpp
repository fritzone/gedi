...
#include "BuildSystem.h"      // buildSystem.guessCompileCommand / get_full_compile_command
#include "EditorBuffer.h"     // buffer.compiler_settings, buffer.filename
#include "utils.h"            // wrap_text, wchar_to_utf8
#include <ncurses.h>
#include <cstdio>             // popen / pclose for clipboard copy
...
// Build the option list for the currently active tab.
static std::vector<OptionList::Option> buildOptions(const CompilerSettings& s, int tab)
{
    using O = OptionList::Option;
    CompilerSettings& t = const_cast<CompilerSettings&>(s);   // refs into mutable temp_
...
CompileOptionsDialog::CompileOptionsDialog(Renderer&     renderer,
                                           ...
                                           EditorBuffer& buffer)
    : DialogBase("Compiler Options", W, H)
    , renderer_    (renderer)
    , buildSystem_ (buildSystem)
    , buffer_      (buffer)
    , temp_        (buffer.compiler_settings)
    , standards_   ({"c++11","c++14","c++17","c++20","c++23","c++26"})
{
    // Find the current standard in the list
...
void CompileOptionsDialog::show(Renderer&     renderer,
                                ...
                                EditorBuffer& buffer)
{
    CompileOptionsDialog dlg(renderer, buildSystem, buffer);
    dlg.run(renderer);
}
...
void CompileOptionsDialog::onInit()
{
    ...
    {
        FocusGroup g;
        g.title  = "";   // no surrounding box — drawn inline by onDraw
        g.hotkey = '\0';
        g.box_w  = 0; g.box_h = 0;  // suppress box drawing
        g.box_x  = 0; g.box_y = 0;
        g.comboboxes.push_back(ComboBox(standards_, std_idx_,
                                        17, COMBO_Y, 12));
        addGroup(std::move(g));
    }
    ...
    // ── Group 1: Tab bar ─────────────────────────────────────────────────────
    {
        ...
        g.box_w  = 0; g.box_h = 0;
        g.box_x  = 0; g.box_y = 0;
        g.tabcontrols.push_back(TabControl({"Basic","Advanced","Expert"},
                                           2, TAB_Y, W - 4));
        addGroup(std::move(g));
    }
    ...
    // ── Group 2: Option list ──────────────────────────────────────────────────
    {
        ...
        // Populate for tab 0 initially; updated via onDraw each frame
    ...
    // ── Group 4: Final command (read-only; Up/Down scroll via onKey) ──────────
    {
        FocusGroup g;
        g.title   = " Final Command ";
        g.hotkey  = '\0';
        g.box_x   = 2;  g.box_y = CMD_BOX_Y;
        g.box_w   = W - 4;  g.box_h = CMD_BOX_H;
        // No widgets — content drawn by onDraw, scrolled by onKey
        addGroup(std::move(g));
    }
        ...
        .buttons = {
            Button{
                ...
                .on_activate = [this]() -> HandleResult {
                    // temp_.cpp_standard is already current (synced every frame
                    // in onDraw), so we can write back directly.
                    buffer_.compiler_settings = temp_;
                    result().accept();
            ...
            Button{
                ...
                .x = W/2 - 6, .y = BTN_Y,
                .on_activate = [this]() -> HandleResult {
                    // Build the final command and copy to clipboard
                    std::string base = buildSystem_.guessCompileCommand(buffer_.filename);
                    std::string cmd  = buildSystem_.get_full_compile_command(base, temp_);
...
void CompileOptionsDialog::onDraw(Renderer& renderer, int startx, int starty)
{
    ...
    // ── Sync ComboBox selection → temp_.cpp_standard (every frame) ─────────────
    // This ensures the Final Command reflects the current ComboBox value
    // live, without waiting for Ok to be pressed.
    if (!groups()[GRP_STANDARD].comboboxes.empty())
        temp_.cpp_standard = groups()[GRP_STANDARD].comboboxes[0].selectedText();

    // ── Sync OptionList content with the currently active tab ────────────────
    // The TabControl lives in group 1; the OptionList in group 2.
    ...
    // The surrounding box is drawn by FocusGroup::draw(); we draw the content.
    ...
    // ── Final command (scrollable, read-only) ─────────────────────────────────
    std::string base    = buildSystem_.guessCompileCommand(buffer_.filename);
    std::string cmd     = buildSystem_.get_full_compile_command(base, temp_);
    ...
    for (int i = 0; i < CMD_VIEW_H; ++i) {
        ...
        // Pad to field width so background colour fills the row
        if ((int)text.size() < W - 8)
            text += std::string(W - 8 - text.size(), ' ');
        renderer.drawText(startx + 4, starty + CMD_BOX_Y + 1 + i,
                          text,
                          cmd_focused ? Renderer::CP_MENU_SELECTED
                                      : Renderer::CP_DIALOG);
    }
}

// ── onKey: Up/Down scroll the command view when it is focused ─────────────────
